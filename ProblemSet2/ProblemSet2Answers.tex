...
\begin{enumerate} [label=\Alph*]
        ...
        \item  Knowing that
          ...
          \item 
          For both the 2x2 and 3x3 the number of channels depends on the number of input channels. In the examples we have only 1 input channel.\\
          For a 2x2 convolution let's use this example of a matrix I and we will need to use 2 F fitlers: \\
          \[
            I = \begin{bmatrix}
                1 & 8\\
                5 & 6 \\
            ...
            we apply each filter to the first 2 columns and we get $1-5=-4$ and $8-6=2$ so 
            by applying a ReLU layer and adding the second element
            on both the results we get that the 5 is the first max and 8 the second max.
            $Result = \begin{bmatrix}
              5 & 8
            \end{bmatrix}$ 
            Now to get the max of theese 2 we use a filter 
            $ F2 = \begin{bmatrix}
              1 & -1
            \end{bmatrix}$ and we get $5-8=-3$ by using a ReLU operation and adding the second element we get that the max is 8.
            We have successfully max pooled a 2x2 convolution using a convolution layer with 2 filters then a ReLU layer and then 
            a convolution layer with 1 filter and then 1 ReLU layer. So in total we have 2 convolutional and 2 ReLU layers.\\

            For a 3x3 convolution let's use this example of a matrix I and we will need to use 3 F fitlers:
            \[
              I = \begin{bmatrix}
                  1 & 8 & 15\\
                  5 & 6 & 18\\
            ...
            We apply each filter to the first and second row of the first 3 columns and we get $1-5=-4$, $8-6=2$ and $15-18=-3$. Then we apply a ReLU
            operation to each result to get the max of each pair.
            Then we do the same for the second and and third rows and we get $5-7=-2$, $6-3=3$, $18-11=7$ 
            $Result = \begin{bmatrix}
              5 & 8 & 18 \\
              7 & 6 & 18
            \end{bmatrix}$ 

            Now we use this filter on the first and second row
            $F = \begin{bmatrix}
              1 & -1
            \end{bmatrix}$
            So we get $5-8=-3$, $7-6=1$. After a ReLU operation and adding the second element we get max 8 and 7.
            Now we compare this with the third column:
            $Result = \begin{bmatrix}
              8 & 18 \\
              7 & 18
            \end{bmatrix}$ 
            we apply the filter both filters again  so $8-18=-10$ and $7-18=-11$. After ReLU operation and adding the second element the max is 18 and 18.
            The last step is to use the filter again: 
            $Result = \begin{bmatrix}
              18 & 18
            \end{bmatrix}$ 
            we get $18-18=0$ and by ReLU and adding the second element we get 18 as the max of all 9 elements.
            By adding all the layers we need we have 4 convolution layers and 4 ReLU layers.
