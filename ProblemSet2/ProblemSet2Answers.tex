...
\noindent 
Write a (MATLAB/python/Keras/…) program to implement the backpropagation
...
Use $S^1=2, S^1=8, s^1=12$  Experiment with several different values for the learning
...
\noindent We experimented with 3 learning rates:
\begin{enumerate}
  \item For a = 0.01: \\
...
\noindent From the graphs we can see that the more neurons a network has the more accurate it is.
There are however diminishing returns so the more neurons we add the less noticable the difference in accuracy.
\\
We can also observe that if the learning rate is too small or too big then the graph is also inacurate.

%Problem 5
\newpage
\noindent \textbf{Problem 5} \\

In the setting with $S^1= 12$ and learning rate a=0.1 of the previous exercise, apply the
...
For each of the $\theta$ values we get the following graphs:
\begin{enumerate}
  \item For $\theta=0.15$ \\
  \includegraphics[width=0.3\textwidth]{Problem5_0.15.png}
  \item For $\theta=0.25$ \\
  \includegraphics[width=0.3\textwidth]{Problem5_0.25.png}
  \item For $\theta=0.35$ \\
  \includegraphics[width=0.3\textwidth]{Problem5_0.35.png}
\end{enumerate}

We can observe the algorithm is not as accurate as in Problem 4 however out goal of preventing overfitting has been accomplished
...
The above equations can represent 3 different optimization methods:
